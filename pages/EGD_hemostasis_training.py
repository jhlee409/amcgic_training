...
if st.session_state.get('logged_in'):     
    ...
    if selected_pre_videos_file:
        ...
        with pre_video_container:           
            with pre_video_container:
                video_html = f'<video width="500" height="100" controls><source src="{st.session_state.pre_video_url}" type="video/mp4"></video>'
        ...
        if st.sidebar.button('진행'):
            if st.session_state.get('selected_video_file'):
                ...
                with video_player_container:
                    video_html = f'<video width="1000" height="700" controls><source src="{video_url}" type="video/mp4"></video>'
                    st.markdown(video_html, unsafe_allow_html=True)
