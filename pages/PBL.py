    ...
    with col2:
        ...
        if selected_case_file:
            ...
            if selected_case_file == "000.docx":
                ...
                # Display Form Title
        main_container.subheader("AMC GI:&emsp;PBL 훈련 챗봇&emsp;&emsp;&emsp;v 1.0")
        with main_container.expander("정상적이 작동을 위해, 반드시 먼저 여길 눌러서 사용방법을 읽어 주세요."):
            st.write("- 처음에는 왼쪽 sidebar에서 증례 파일을 선택해 주세요.")
            st.write("- case가 준비되면 맨 처음은 '로딩'을 입력하세요. 관련 자료를 로딩해야 하고, 좀 오래 걸립니다 ^^;")
            st.write("- 처음부터 앞에 뭐가 많이 쌓여 있으면 왼쪽에 있는 '이전 대화기록 삭제버튼'을 눌러 청소하세요.")
