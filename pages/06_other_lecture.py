...
st.set_page_config(page_title="other_lecture", layout="wide") 
...
# Display Form Title
st.subheader("other_lecture 강의 모음음")
with st.expander(" 필독!!! 먼저 여기를 눌러 사용방법을 확인하세요."):
    st.write("- 이 강의 모음은 여러 종류의 강의를 모아둔 폴더입니다.")
...
if st.sidebar.button("본강의 시청"):
    ...
    if selected_lecture != "Default":
        ...
        log_entry = f"Position: {position}, Name: {name}, Access Date: {access_date}, other_lecture: {selected_lecture}\n"

        log_blob = bucket.blob(
            f'log_other_lecture/{position}*{name}*{selected_lecture}'
        )
