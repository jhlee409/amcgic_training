...
if st.session_state.get('logged_in'):
    ...
    with st.expander(" 필독!!! 먼저 여기를 눌러 사용방법을 확인하세요."):
        ...
        st.write("- 그 아래에서 EGD 사진을 선택해서 업로드 하세요.")
        st.write("- 다음 그 아래에서 case instruction 파일을 upload 하세요.")
        st.write("- '질문을 시작해도 될까요?'라고 나오면 '예'라고 대답하시면, 간단한 사진 설명과 함께 질문이 나옵니다.")
        st.write("- 질문에 대한 대답을 입력하고 잠시 기다리면 권장 답안과 비교하여 평가해 줍니다. 이어서 선생님에게 추가 질문을 2개 정도 더 합니다. 그에 대해 답을 하면 권장 답안과 비교해 줍니다.")
        st.write("- 한 증례가 끝나고 다음 증례로 넘어가시려면, 먼저 '초기화' 버튼을 선택하여 초기화 하고, 처음 과정부터 다시 시작하시면 됩니다.")
        st.write("- 각 단계마다 반드시 '열일 중' 스핀이 멈출 때까지 기다리세요. 스핀 돌 때 다른 버튼 누르면 오류납니다.")
        st.write("- Impression을 적는 질문에 대한 대답은 실제 report에 적는 impression을 적으셔야 합니다. 의뢰자가 impression만 보는 경우가 많으므로, 추정 진단뿐 아니라 의미있는 항목은 모두 적어야합니다.")
        st.write("----- 진료에 도움이 되는 의미있는 정보이기 때문에, organ, 부위, 위치, 크기, 형태 분류를 알 수 있으면 반드시 써야 합니다. 예; gastric MB PW, 1.8 cm, IIa type, TA")
        st.write("----- 이 AI는 의미있는 항목은 평가하고, 의미없는 항목은 평가하지 않도록 코딩되어 있습니다. 당연하게 질문이나 사진에서 알 수 없는 정보는 적을 필요 없고, 평가도 하지 않습니다.")
