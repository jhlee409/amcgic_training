...
def generate_video_url(file_path):
    ...
    data = [
        {"text": "정상 위에서 Expert의 검사 전과정 B", "videos": ["EGD_variation/B1.mp4", "EGD_variation/B2.mp4"]},
        {"text": "STG with Bilroth II reconstruction state애서 검사 전과정 C", "videos": ["EGD_variation/C1.mp4", "EGD_variation/C2.mp4"]},
        {"text": "STG with Bilroth I reconstruction state에서 검사 전과정 D", "videos": ["EGD_variation/D1.mp4", "EGD_variation/D2.mp4"]},
        # Add more items as needed...
    ]

    # Cache URLs
    for item in data:
        item["video_urls"] = [generate_video_url(video) for video in item["videos"]]
    ...
    # Attendance Button
    if st.button("출석 확인"):
        user_email = st.session_state.get('user_email', 'unknown')  # 사용자 이메일
        access_date = datetime.now().strftime("%Y-%m-%d")  # 접속 날짜
        log_entry = f"Email: {user_email}, Menu: EGD Variation, Access Date: {access_date}\n"
        log_blob = bucket.blob(f'log_EGD_varation/{user_email}_EGD Variation_{access_date}.txt')
        log_blob.upload_from_string(log_entry, content_type='text/plain')
        st.success("출석이 확인되었습니다!")

    # Instructions
    with st.expander("필독!!! 먼저 여기를 눌러 사용방법을 확인하세요."):
        st.write("- 먼저 '가장 먼저 보세요' 동영상을 시청하세요.")
        st.write("- 상황에 맞는 전문가의 해설 동영상을 필요할 때 골라 보시면 됩니다.")

    # Display data with video links
    for item in data:
        col1, col2, col3 = st.columns([4, 1, 1])
        with col1:
            st.write(item["text"])  # Display text
        for idx, video_url in enumerate(item["video_urls"], start=1):
            with [col2, col3][idx - 1]:
                if st.button(f"Link {idx}"):
                    video_placeholder.video(video_url)  # Play video
