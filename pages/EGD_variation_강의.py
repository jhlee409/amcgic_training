...
if st.session_state.get('logged_in'):
    ...
    with st.expander(" 필독!!! 먼저 여기를 눌러 사용방법을 확인하세요."):
        st.markdown('''
            해당 주제에 대해 여러 증례를 대상으로 해설하는 동양상의 버튼이 오른쪽이 있습니다. 버튼을 눌러 동영상을 시청하세요.
            전체에 대한 안내와 가장 먼저 보세요: 전체 과정 해설 강의에 있으므로 제일 먼저 시청하세요.
            ...
            if idx < 5:  # 버튼은 최대 5개까지만 표시
                with cols[idx + 1]:  # 두 번째 열부터 버튼 추가
                    ...
                    if st.button(f"{video_name}"):
                        ...
                        log_blob = bucket.blob(f'log_EGD_varation/{user_email}_EGD variation_{video_name}.txt')  # 로그 파일 경로 설정
